...
const addGuestReservation = async (req, res) => {
    ...
    try {
        ...
        const values = [activity_name, activity_desc, room_id, admin_id, start_datetime, end_datetime, discount, additional_fee, total_amount_due, status_code]
        ...
        const insertnotifResult = await conn.query(`INSERT INTO reservation_notification(reservation_id, actor_id, status_code) VALUES(?,?,?)`, [result.insertId, admin_id, status_code])
        ...
        res.send({success:true, transaction_id: transaction_id, message: "Successfully added Guest reservation." })
    } catch (err) {
        console.log(err);
        await conn.rollback();
