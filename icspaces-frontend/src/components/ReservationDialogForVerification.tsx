...
const ReservationDialogForVerification: React.FC<ReservationDialogProps> = ({
  open,
  onClose,
  reservation,
}) => {
  ...
  return (                  
    <Dialog open={open} onClose={onClose} classes={{ paper: 'dialog-paper' }} sx={{'& .dialog-paper': {borderRadius: '25px', padding: '20px', transform: 'scale(0.8)'}}}>
      ...
      <DialogContent sx={{ overflow: 'hidden'}}>
        {reservation && (
          <>
            ...
            <Grid container spacing={0}  columns={40} padding={2}>      
              ...
              <Box border={1} paddingTop={3} style={{ width: '100%', border:'none'}}>
                <Typography className="unselectable" sx={{textAlign: 'left', fontSize: '0.9vw', padding:'0px', lineHeight: '1.3', display: 'block', color:SCHEME_FONT_DEFAULT_COLOR, paddingBottom: '13px'}}>
                  Your reservation is currently being processed for verification. An email will be sent to <span style={{ color: '#8eafc1' }}>{reservation.user_id}</span> shortly. Please wait for further updates regarding your reservation.
                </Typography>
              </Box>

              {/* Spacer line */}
              <Grid item xs={40} sx={{paddingTop:'25px'}}>
              ...
              <Grid item xs={20} sx={{paddingTop:'15px'}}>
              <Button variant="contained" style={{ backgroundColor: '#d9d9d9', color: '#183048', borderRadius: '25px', width: '100%', 
            ...
            {actionTaken === "cancel" && (
              <ReservationDialogPopupCancel open={cancelledDialogOpen} onClose={() => setCancelledDialogOpen(false)} reservation={reservation} />
            )}
...
export default ReservationDialogForVerification;
