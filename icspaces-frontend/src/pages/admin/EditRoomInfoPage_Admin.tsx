...
const EditRoomInfoPage_Admin = () => {
  ...
  const [newItem, setNewItem] = useState<Utility>({
    item_name: "",
    item_qty: 0,
    fee: 0,
  });
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const utilityResponse = await fetch(
        "https://api.icspaces.online/set-utilities",
        {
          method: "POST",
          headers: {
  ...
  const getRoomInformation = async () => {
    try {
      ...
      if (roomInfo && roomData.utility) {
        ...
        const initialQuantities = roomData.utility.map(
          (item: Utility) => item.item_qty
        );
  ...
  useEffect(() => {
    if (roomOvertimeFee.current) {
      roomOvertimeFee.current.value =
        room?.additional_fee_per_hour !== undefined
          ? room.additional_fee_per_hour.toString()
          : "";
    }
    if (roomFee.current) {
      roomFee.current.value =
        room?.fee !== undefined ? room.fee.toString() : "";
    }
    if (roomName.current) {
                ...
                <>
                  ...
                  <div>
                    <TextField
                      ...
                      error={roomNameError}
                      helperText={
                        roomNameError ? "This field is required." : ""
                      }
                  ...
                  <TextField
                    ...
                    helperText={
                      hourlyError
                        ? "This field is required. Please add valid integer inputs."
                        : ""
                    }
                    ...
                    InputProps={{
                      ...
                      inputProps: {
                        ...
                        step: 50, // can set step value
                        max: 100000,
                      },
                    }}
                  />
                  <TextField
                    id="room-overtime-fee"
                    label="Additional Fee per Hour"
                    // defaultValue={room?.additional_fee_per_hour}
                    // defaultValue={room?.additional_fee_per_hour !== undefined ? room.additional_fee_per_hour.toString() : ''}
                    variant="outlined"
                    size="small"
                    type="number"
                    inputRef={roomOvertimeFee}
                    InputLabelProps={{ shrink: true }}
                    error={overtimeError}
                    helperText={
                      overtimeError
                  ...
                  {items.map((item, index) => (
                    <ListItem key={index} style={styles.listItem}>
                      ...
                      <TextField
                        ...
                        type="number"
                        inputRef={(input) =>
                          (utilityPrice.current[index] = input)
                        }
                        InputProps={{
                          style: {
                            ...inputStyle,
                      ...
                      <TextField
                        ...
                        type="number"
                        inputRef={(input) =>
                          (utilityQty.current[index] = input)
                        }
                        InputProps={{
                          style: {
                            ...inputStyle,
